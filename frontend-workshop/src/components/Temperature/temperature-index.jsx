...
import TemperatureTable from './temperature-data';
...
const temperatureIndex = () => {
  ...
  useEffect(() => {
    ...
    function resetInactivityTimeout() {
      ...
      window.inactivityTimeout = setTimeout(() => {
        ...
        fetch('http://localhost:5000/temperatureData').then((response) => {
          if (response.ok) {
            console.log('Reload');
            console.clear();
  ...
  return (
    <Tabs isFitted variant="soft-rounded" colorScheme="green">
      <TabList>
        <Tab>Temperature Level Data</Tab>
      </TabList>
      <TabPanels>
        <TabPanel>
          <TemperatureTable />
